...
export const deleteListing = async (listingId) => {
    const response = await fetch(`${listingsDomain}/listings/${listingId}`, {
        method: "DELETE",
        headers: appwriteHeaders
    })
    if (!response.ok) throw new Error(response.statusText)
...
export const editListing = async (listing) => {
    const response = await fetch(`${listingsDomain}/listings/${listing.$id}`, {
        method: "PUT",
        body: JSON.stringify(listing),
        headers: { ...appwriteHeaders, "Content-Type": "application/json" }
    })
    const editedListing = await response.json()
    console.log("EDIT LISTING RESULT", result)
