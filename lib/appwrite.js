...
export const uploadFile = async (file, type, listingId) => {
    ...
    try {
        ...
        if (listingId) {
            await databases.createDocument(config.databaseId, config.imagesCollectionId, ID.unique(), { listing: listingId })
        }
    ...
    try {
        ...
        if (form.images.length > 0) {
            ...
            for (let image of form.images) {
                let imageId = await uploadFile(image, 'image', listing)
                images.push({ imageId })
            }
        }
        return true
