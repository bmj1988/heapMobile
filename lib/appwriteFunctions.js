...
export const fetchFeed = async (long, lat, userId, radius, page = 1) => {
    if (!long || !lat || !userId || !radius || !page) return []
    const response = await fetch(`https://${process.env.EXPO_PUBLIC_LCOATION_FUNCTION_DOMAIN}.appwrite.global/feed?page=${page}&radius=${radius}`, {
        method: "GET",
