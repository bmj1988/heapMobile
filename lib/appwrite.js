...
export const getUserListings = async (userId) => {
    try {
        const posts = await databases.listDocuments(config.databaseId, config.listingsCollectionId, [Query.equal('users', userId)])
        return posts.documents;
    }
    catch (e) {
        throw new Error(e)
...
export const getUserBids = async (userId) => {
    try {
        const posts = await databases.listDocuments(config.databaseId, config.bidsCollectionId, [Query.equal('users', userId)])
        return posts.documents;
    }
    catch (e) {
        throw new Error(e)
